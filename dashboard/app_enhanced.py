...
def render_paper_card(row: pd.Series):
    badge = f"<span style='background:#1f4788;color:#fff;padding:2px 6px;border-radius:6px;font-size:0.8rem;'>Relevance {row.get('relevance_score',0):.0f}</span>"
    st.markdown(f"**{row.get('title','(untitled)')}**  {badge}", unsafe_allow_html=True)
    st.caption(
        f"   {row.get('university','')} |   {str(row.get('theme','')).replace('_',' ').title()} |   {pd.to_datetime(row.get('date')).strftime('%Y-%m-%d') if pd.notna(row.get('date')) else ''}"
    )
    with st.expander("View Details"):
        ...
        # Try to detect a PDF URL
        pdf_url = None
        for key in ["pdf_url", "oa_pdf", "open_access_pdf", "pdf", "primary_pdf_url"]:
            val = str(row.get(key, "")).strip()
            if val and val != "nan":
                pdf_url = val
                break
        if not pdf_url and isinstance(link, str) and link.lower().endswith('.pdf'):
            pdf_url = link
        # Provide Open/Download PDF actions if available
        if pdf_url:
            st.markdown(f"[  Open PDF]({pdf_url})")
            try:
                dl_col1, dl_col2 = st.columns([1,3])
                if dl_col1.button("Download PDF", key=f"dl_{hash(pdf_url)}"):
                    with st.spinner("Downloading PDF..."):
                        ...
                        if resp.ok and resp.headers.get('Content-Type','').lower().startswith('application/pdf'):
                            st.download_button(
                                label="   Save PDF",
                                data=resp.content,
                                file_name=(row.get('title','paper').replace(' ','_')[:60] + ".pdf"),
                                mime="application/pdf",
                                key=f"save_{hash(pdf_url)}"
                            )
                        else:
                            st.info("PDF not directly downloadable. Use 'Open PDF' above.")
            except Exception:
                st.info("PDF download not available; use 'Open PDF' link above.")
...
if page == "  Overview":
    ...
    # Count papers per theme and include all Babcock themes (zero-filled)
...
# Trends Over Time
    ...
    if picks:
        ...
        # Build full grid of quarters x selected themes and fill zeros
